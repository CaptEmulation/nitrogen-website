---
title: A Nitrogen controllable light
---

## A Nitrogen controllable light

Now that you've [setup Nitrogen](/guides/temperature/setup.html) and [built a virtual thermometer](/guides/temperature/thermometer.html), the next step is to build a device that can respond to incoming commands.

We're going to work against the hosted version of Nitrogen but with just a configuration change, you can run the same device against your own instance of Nitrogen.

The device app, like the [thermometer device app](/guides/temperature/thermometer.html) we built in the last guide is a node.js application that can run anywhere that node.js can run. For this exercise we'll be running it from the command line but you can run this same code on an Arduino Yun or a Raspberry Pi.

For simplicity sake, we're going to write this all in one JavaScript file with a package.json that pulls in all of our dependencies. When your devices get more sophisticated, it will certainly want to split them up into separate files and possibly even separate NPM packages.

First, create a folder called light:
...
There are 2 NPM packages that we need. The first is the Nitrogen client library that we'll use to interact with the Nitrogen service. The second is the file based Nitrogen store. This is the library that handles storing your credentials and remembering who this device is for the next time that you connect to the service.
...
Now we're ready to write some code. Create a file in your favorite text editor and save it as light.js.
...
And with that, you have a Nitrogen device connected to the service. However, this is not an interesting device as it doesn't send any telemetry and doesn't receive any commands.
...
service.connect(light, function(err, session, light) {
    ...
    var message = new nitrogen.Message({
        ...
        body: {
            command: {
                message: "Light (" + light.id + ") is on at " + Date.now()
            }
        }
    });
...
If you take a look at the directory that your code is in, there's a new folder called <hostname>\_<portname> or in our case `api.nitrogen.io_443`. If you look at this file, you can see that it contains your newly minted credentials in a simple JSON format. Nitrogen has a pluggable store system that can integrate more complex storage schemes for credentials.

At this point you have a device that is authenticated and can send telemetry data up to Nitrogen's public gateway.

### Receiving commands.

The next thing to do is to start receiving commands from the server by implementing a [Nitrogen CommandManager](/docs/nitrogen/commandManager.html).
...
The next method is a little trickier. Messages that you have already processed or are no longer interesting to you, such as one that has expired, can be filtered out in the "obsoletes" method. It's important to do this correctly or you will keep getting the same queue of messages over and over.
...
Now that we've figured out that it's a message that we're actually interested in and that is still relevant, we'll get a queue of these message that we need to execute look through and execute commands based on. In our case, we're just going to send a message back to the server stating that the message was received and that the light is either in the on or off state now. It's an exercise to the reader to get it to actually control the hardware aspects of a light on your platform but there are lots of tutorials for that work.
...
lightManager.prototype.executeQueue = function(callback) {
    ...
    var message = new nitrogen.Message({
        ...
        body: {
            command: {
                message: "Light (" + light.id + ") is " + JSON.stringify(lightOn) + " at " + Date.now()
            }
        },
...
service.connect(light, function(err, session, light) {
    ...
    new lightManager(light).start(session, function(err, message) {
...
Now if you run your app, you'll see the see that it authenticates and then goes into a ready state.
